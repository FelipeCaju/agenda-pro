...
SET @sql := IF(
  @has_appointments_quote_id = 0,
  'ALTER TABLE appointments ADD COLUMN quote_id CHAR(36) NULL AFTER resposta_whatsapp',
  'SELECT 1'
);
...
SET @has_appointments_service_order_id := (
  SELECT COUNT(*)
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'appointments'
...
SET @sql := IF(
  @has_appointments_service_order_id = 0,
  'ALTER TABLE appointments ADD COLUMN service_order_id CHAR(36) NULL AFTER quote_id',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
