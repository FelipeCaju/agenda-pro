...
function mapAppSettings(row) {
  ...
  return {
    ...
    recurring_whatsapp_automatico: toBoolean(row.recurring_whatsapp_automatico ?? 1),
...
async function ensurePlatformSettingsInfrastructure() {
  if (await hasTable("app_settings")) {
    ...
    if (!(await hasColumn("app_settings", "recurring_whatsapp_automatico"))) {
      await execute(
        `ALTER TABLE app_settings
          ADD COLUMN recurring_whatsapp_automatico TINYINT(1) NOT NULL DEFAULT 1 AFTER whatsapp_tempo_lembrete_minutos`,
      );
    }

    if (!(await hasColumn("app_settings", "recurring_marcar_vencido_automaticamente"))) {
      await execute(
        `ALTER TABLE app_settings
          ADD COLUMN recurring_marcar_vencido_automaticamente TINYINT(1) NOT NULL DEFAULT 1 AFTER recurring_whatsapp_automatico`,
      );
    }

    if (!(await hasColumn("app_settings", "recurring_whatsapp_template"))) {
      await execute(
        `ALTER TABLE app_settings
...
async function ensureRecurringInfrastructure() {
  ...
  await execute(
    `CREATE TABLE IF NOT EXISTS recurring_charges (
      ...
      UNIQUE KEY uq_recurring_charges_org_profile_reference (organization_id, recurring_profile_id, referencia_data_cobranca),
...
async function ensureInitialized() {
  ...
  await ensureRecurringInfrastructure();
