      ...
      <div className="mx-auto flex min-h-14 items-center justify-between gap-3 xl:min-h-[76px] xl:max-w-none xl:items-end">
        <div className="flex min-w-0 flex-1 items-center gap-2 xl:gap-3">
          ...
          <div className="min-w-0">
            ...
            {subtitle ? (
              <p className="truncate text-xs text-slate-500 xl:mt-1 xl:max-w-[52rem] xl:text-sm">{subtitle}</p>
            ) : null}
          </div>
        </div>
        {action ? <div className="ml-2 shrink-0 xl:self-center">{action}</div> : null}
      </div>
    </div>
  );
}
